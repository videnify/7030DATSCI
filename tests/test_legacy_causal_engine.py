"""
test_legacy_causal_engine.py
=============================
LEGACY / UNUSED CODE -- renamed from test_event_study.py on 2026-07-15.

Unit tests for `EventStudy` in `src/causal_engine.py`: an OLS market-model
(alpha + beta regression) event-study implementation. This is NOT the
methodology used anywhere in the current pipeline -- `notebooks/04_causal_
analysis.ipynb` implements a different, inline constant-mean-return model
(no regression), with different default windows. See
`docs/research_bible/03_methodology.md` (which itself flags `causal_engine.py`
as unused) and `docs/research_bible/10_decision_log.md`, 2026-07-15 entry.

Kept, renamed rather than deleted, because `src/causal_engine.py::EventStudy`
is a genuinely independent, self-consistent implementation of the classical
Fama/Fisher/Jensen/Roll market-model event study -- useful as a reference or
future robustness check even though it does not validate any currently
reported result. The active methodology is tested separately in
`tests/test_event_study.py` against `src/event_study_reference.py`.
"""
