...
**DATSCI7030 — Causal Event-Driven Market Impact Modelling** · Version 2.3 (2026-07-13)
...
**Last updated:** 2026-07-13 (GDELT lineage gap fix) — `data_lineage.svg` was missing the GDELT path entirely: `gdelt_daily_summary.parquet` (raw, notebook 1) and `gdelt_daily_risk.parquet` (processed, notebook 3) are both genuinely read by `scripts/build_master_dataset.py` to build `master_dataset.parquet`, but the diagram predated this session's GDELT integration work and never included them. Added both as a fifth raw-source box and a third row-2 box, with their own connector path directly into `master_dataset.parquet` — drawn separately from the `events_tagged.parquet`/`daily_sentiment.parquet` path since GDELT is a continuous confounder that never passes through the event catalogue (per the 2026-07-13 Phase 0 decision). Canvas widened 980→1030px to fit the fifth raw box; re-verified zero text overflows and zero new line crossings using the same measurement scripts as the prior pass. Version bumped to v2.3.
...
| `data_lineage.svg` | Raw files → `events_tagged.parquet`/`daily_sentiment.parquet` → `master_dataset.parquet` → `car_results.parquet` → `feature_matrix.parquet` → models/reports, plus a `causal_estimates.parquet` branch (added 2026-07-08) showing the causal-analysis output separately, since it is *not* consumed by feature engineering, and a `gdelt_daily_summary.parquet` → `gdelt_daily_risk.parquet` branch (added 2026-07-13) showing GDELT's own path into `master_dataset.parquet`, separate from the event catalogue | Chapter 3 §3.5 (Feature Engineering) — kept in the dissertation |
