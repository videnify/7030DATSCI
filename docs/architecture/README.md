...
Earlier: 2026-07-14 (Visualisation v1.1 and dissertation synchronization) — Notebooks 01–08, the validated baseline/event suite, RF/SHAP, RQ2/RQ3 comparison and figures 08a–08d were synchronized. Version 2.9.

Earlier: 2026-07-14 (FES v1.1 model-evaluation freeze) — Notebooks 06–07 complete while Notebook 08 figures remained pending. Version 2.8.

Earlier: 2026-07-14 (`Baseline_LASSO` v1.1 freeze) — Notebook 06 complete, Notebook 07 event models/RQ2 outputs and Notebook 08 figures pending. Version 2.7.

Earlier: 2026-07-14 (FES v1.1 freeze) — the diagrams identified the 92-feature matrix and validation `PASS`, with Notebooks 06–08 pending. Version 2.6.

Earlier: 2026-07-14 (Dataset v1.2 freeze) — `data_lineage.svg` identified the frozen 34-column Dataset v1.2 while FES v1.1 was still pending. Version 2.5.

Earlier: 2026-07-14 (Notebook 03 / Dataset v1.2 preparation) — added the `daily_sentiment.parquet` sentiment + direct occurrence-count annotation and marked Dataset v1.2 as pending. Version 2.4.

Earlier: 2026-07-13 (GDELT lineage gap fix) — `data_lineage.svg` was missing the GDELT path entirely: `gdelt_daily_summary.parquet` (raw, notebook 1) and `gdelt_daily_risk.parquet` (processed, notebook 3) are both genuinely read by `scripts/build_master_dataset.py` to build `master_dataset.parquet`. Added both as a fifth raw-source box and a third row-2 box, with their own connector path directly into `master_dataset.parquet`, separate from the event catalogue. Version 2.3.

Earlier: 2026-07-13 (Visual Enhancement pass) — all six diagrams moved to a lighter, softer palette (steel-blue accent `#3A6EA5`→`#5B8FC7`, box fill `#EEF3F8`→`#F3F7FC`, all text/divider/stroke tones lightened correspondingly; stroke-widths reduced ~20% for a less heavy line weight). Two real defects were found by measuring actual rendered text width against every box (using the real Arial/Arial Bold font metrics, not visual guessing) and fixed: `data_lineage.svg`'s `app_presidential_documents.parquet` box was overflowing its label (widened 163px→225px, connector repositioned to match) and `rq_traceability.svg`'s RQ3 subtitle was overflowing (shortened to `Event_LASSO / XGBoost / LightGBM`, smaller font). A genuine connector-line crossing in `rq_traceability.svg` (the Causal Analysis→Results and SHAP→Results paths crossed each other, found by exact line-segment-intersection geometry, not eyeballing) was fixed by swapping which path uses the outer vs. inner routing corridor and which entry point on the Results box it uses — the two paths now nest instead of crossing. All fixes were verified programmatically (zero text overflows, zero line crossings across all 6 files) before being accepted. See `docs/research_bible/10_decision_log.md` (2026-07-13 entry) for the full record. Earlier: 2026-07-08 (Final Appendix Cleanup & Architecture SVG Enhancement mission) — `car_formula.svg` was factually corrected and restyled to match the other five diagrams (it was previously the visual outlier: black borders, gray arrows, no italic subtitle, and content that contradicted frozen Section 3.3); `data_lineage.svg` gained a `causal_estimates.parquet` branch and two minor label/spacing fixes; `rq_traceability.svg` had its RQ1/RQ2 connector lines rerouted around box edges instead of through box text. Earlier still: 2026-07-06 (Architecture SVG Cleanup & Rebuild mission) — the full diagram set was audited; 11 diagrams describing an earlier, superseded ARIMA/intervention-analysis methodology were archived, 2 broken/misattributed files were deleted, and the 5 canonical diagrams (now 6, including `car_formula.svg`) were rebuilt from scratch against the current frozen pipeline.
...
| `data_lineage.svg` | Raw files → `events_tagged.parquet`/`daily_sentiment.parquet` → `master_dataset.parquet` → `car_results.parquet` → `feature_matrix.parquet` → models/reports. `daily_sentiment.parquet` is explicitly labelled as sentiment + occurrence counts (2026-07-14); the GDELT and causal-estimate branches remain separate by design. | Chapter 3 §3.5 (Feature Engineering) — kept in the dissertation |
| `research_governance.svg` | Research Questions → Hypotheses → SAP v1.0 → Dataset Contract → FES v1.1 → MCP v1.0 → Results | Chapter 3 (Methodology) — governance/versioning figure (not currently embedded) |
