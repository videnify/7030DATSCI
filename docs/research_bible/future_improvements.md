...
29. **No executable notebook reproduces the frozen MCP v1.0 model-comparison result.** `notebooks/05_feature_engineering.ipynb`, `06_model_training.ipynb`, and `07_model_evaluation.ipynb` are all dated 2026-05-31 and implement the **legacy, pre-freeze** pipeline (`model_features.parquet`, a single undifferentiated LASSO/XGBoost/LightGBM comparison, no baseline). The actual frozen `Baseline_LASSO` (`models/baseline/`) and retrained `Event_LASSO`/`XGBoost`/`LightGBM` (`models/event/`) that the dissertation's RQ3 verdict (H0₃ not rejected) rests on were produced **out-of-band** — no notebook or script in this repository reproduces that training run end-to-end. This is a genuine reproducibility gap, not a cosmetic one: an examiner attempting to re-run "the pipeline" from `notebooks/` alone would reconstruct the *legacy* result, not the frozen one. Rebuilding `05`–`07` to actually consume `feature_matrix.parquet`/`feature_contract.md` and reproduce `Baseline_LASSO`/`Event_LASSO` exactly would be a substantive code change carrying real risk of producing numbers that don't match the already-frozen, already-cited results — correctly out of scope for a documentation/style alignment pass. Flagged here as the single highest-priority item in this list; `10_decision_log.md` (2026-07-06) has the full analysis.
