...
26. **GDELT's "5-day sample" is a recent live pull (2026-05-02→05-06), not a historical sample from within 2015–2025.** Confirmed by the new date-range validation check. Strengthens the case in `11_limitations.md` L7 to exclude GDELT-derived features from any "final" result until a genuine historical backfill exists — not actioned here. **✅ Resolved (2026-07-13):** the historical backfill is complete (`data/raw/gdelt_daily_summary.parquet`, 2015–2025, 4,018 daily rows), integrated into `master_dataset.parquet` v1.1 as a continuous confounder, and the contract/version-doc sweep is done (`dataset_version.md`, `dataset_contract.md`, `11_limitations.md` L7, `00_project_overview.md`, `03_methodology.md`, `05_data_dictionary.md`, `08_figures_plan.md`, `15_traceability_matrix.md`, `14_project_dashboard.md`, root `README.md` all updated same day). **Remaining, not yet actioned:** GDELT is still not feature-engineered into `feature_matrix.parquet` (a deliberate v1.0 scope boundary — would need a new FES version to add it). **The `03_event_detection.ipynb` vs. `03_event_detection_revised.ipynb` question is resolved (2026-07-14)** — `_revised` confirmed canonical (it's the only one `src/event_detector.py` is imported by), the pre-rebuild file archived, `_revised` renamed to `03_event_detection.ipynb`. See `14_project_dashboard.md` and `10_decision_log.md`.

## Newly identified (Architecture SVG Cleanup, 2026-07-06)

27. **A proper DoWhy causal DAG diagram is worth rebuilding once the GDELT scope question closes.** Both archived `causal_dag_dowhy*.svg` files depicted GDELT/Goldstein scale as a core input to the causal model, which it isn't (confounders are VIX regime and prior-day return, per `03_methodology.md` §2) — not rebuilt as one of the 5 canonical diagrams this session since it wasn't in the requested set and would need to wait on `11_limitations.md` L7/L8's resolution to avoid the same inaccuracy. **Note (2026-07-13):** per `10_decision_log.md`'s 2026-07-13 GDELT entry, the current Phase 2 plan keeps the DoWhy DAG structure exactly as documented (VIX regime + prior-day return only) and treats GDELT as a continuous feature/confounder candidate outside that frozen graph — so this diagram's existing "GDELT is not a DAG input" framing is still the accurate one to rebuild toward, not something the backfill should change.
...
31. ~~**Figures `08c_predictive_pipeline.png`/`08d_full_dashboard.png` are confirmed stale.**~~ **✅ Resolved (2026-07-14, FES v1.1 final consistency pass).** Notebook 08 now reads the validated FES v1.1 baseline/model-comparison outputs, regenerates all four 08 figures, writes a hash-bound validation manifest with `PASS`, and frames XGBoost as a non-constant diagnostic rather than a validated winner. The figures and dissertation consumers were refreshed together.

---

## Explicitly deferred (do not action before submission)

- Any change to `master_dataset.parquet`, `feature_matrix.parquet`, SAP v1.0, FES v1.1, or MCP v1.0.
