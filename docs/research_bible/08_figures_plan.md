...
| `01_gdelt_distributions.png` | RQ1 (context) | GDELT distributions — as of 2026-07-13 sourced from the full 2015–2025 daily history (4,018 days), not the earlier 5-day sample; still caveat that no GDELT-derived feature is yet used in an RQ2/RQ3 model (`11_limitations.md` L7) |
...
| Figure | Supports | Notes |
|--------|----------|-------|
| `06a_model_comparison.png` | RQ3 historical pre-baseline evidence | ⚠️ Superseded; current baseline-controlled evidence is in Notebook 07 and figures `08c`–`08d` |
...
| `08b_causal_evidence.png` | **RQ1 (summary evidence)** | ✅ Visualisation v1.2: event-type panel reports n, BH q and Cohen's d (0/5 rejected; min q=0.581; max |d|=0.239); pooled DoWhy estimate remains separately positive. The DoWhy panel labels the combined APP + FOMC treatment and does not repeat the persisted legacy method label. |
| `08c_predictive_pipeline.png` | **RQ2 + RQ3 (summary evidence)** | ✅ Current FES v1.1 figure. Shows the complete RF importance, held-out SHAP, four-model comparison, and the RQ2/RQ3 null verdicts from validated Notebook 07 outputs. |
| `08d_full_dashboard.png` | All three RQs | ✅ Current FES v1.1 integrated dashboard. XGBoost is labelled as a diagnostic model selected by defined test IC, not as a winner; the dashboard explicitly states H0₃ is not rejected. |
...
## Baseline-dependent figure refresh record (`07_model_plan.md`)

**Resolved at the current boundary on 2026-07-15 (Visualisation v1.2).** Figures 08c/08d retain the validated four-model FES v1.1 comparison. Figure 08b additionally consumes `event_type_statistics.parquet` and displays the completed RQ1 multiplicity/effect-size result. `results_visualisation_validation.json` is the binding figure manifest.
