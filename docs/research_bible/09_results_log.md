...
- **GDELT:** ~~`gdelt_risk_score`/`gdelt_mean_tone`/`gdelt_n_events` are exactly zero (mean = std = 0) across all 2,765 rows — reconfirms L7 at full-dataset scale, not just the 5-day sample scale.~~ **WITHDRAWN 2026-07-15 — see the dated correction entry at the end of this log.** This claim does not hold: those three columns exist in `data/processed/master_dataset.parquet` (2,765 rows) with real, non-zero variance, not in `notebooks/02_eda.ipynb` §9 as cited, and no computation of this statistic exists in that notebook.
...
**Scope clarification (added 2026-07-15):** this entry's heading and Source line name `master_dataset.parquet`, but the EDA in `notebooks/02_eda.ipynb` (cell 6/34, variable `master_df`) actually loads `data/raw/daily_modelling_calendar_v1.parquet` (2,743 rows × 31 columns) — a Notebook 01 precursor checkpoint, not the later canonical `data/processed/master_dataset.parquet` (2,765 rows × 34 columns, frozen 2026-07-14). The two are closely related (the canonical file supersedes and extends the precursor) but are not the same artefact, and the row/column counts differ. This is a citation/scope correction only: it does not invalidate this entry's stationarity, normality, correlation, or ACF/PACF findings (all computed correctly on whichever file was actually loaded), and it does not change any later frozen `master_dataset.parquet`, `feature_matrix.parquet`, or model result — those were built and validated independently by `scripts/build_master_dataset.py` and Notebooks 05–08. No RQ1, RQ2, or RQ3 result is affected. This clarification is separate from, and does not reopen, the GDELT zero-variance claim withdrawn elsewhere in this entry (see the dated correction entry at the end of this log).
...
## 2026-07-15 — GDELT traceability correction: withdrawal of the 2026-07-05 zero-variance claim

**RQ:** N/A — traceability/governance correction, not a new result. No RQ1–RQ3 conclusion depends on the withdrawn statement.

**Source:** `notebooks/01_data_collection.ipynb` (Section 7/8, GDELT ingestion and calendar build); `notebooks/02_eda.ipynb` (full cell-by-cell inspection); `notebooks/03_event_detection.ipynb` §5; `scripts/build_master_dataset.py`; `data/processed/master_dataset.parquet`; `data/processed/gdelt_daily_risk.parquet`; `data/raw/gdelt_daily_summary.parquet`.

**Decision:** Withdraw the 2026-07-05 Mission 05A claim (above, struck through) that `gdelt_risk_score`/`gdelt_mean_tone`/`gdelt_n_events` have mean = standard deviation = 0 across 2,765 rows, citing `notebooks/02_eda.ipynb` §9.

**What was actually found:**
1. **The cited source is wrong.** `notebooks/02_eda.ipynb` contains no cell — in source or saved output — that computes or prints a mean/std for `gdelt_risk_score`, `gdelt_mean_tone`, or `gdelt_n_events`. Its own "Master Calendar" dataframe (cell 34, §10) is loaded from `data/raw/daily_modelling_calendar_v1.parquet` (2,743 rows × 31 columns) and exposes different GDELT column names entirely (`gdelt_event_count`, `gdelt_mean_goldstein`, `gdelt_mean_tone`) — only `gdelt_mean_tone` overlaps with the withdrawn claim's names.
2. **The cited variable names are not wrong, but they live in a different artefact.** `gdelt_risk_score`, `gdelt_mean_tone`, and `gdelt_n_events` do exist as real column names — in `data/processed/master_dataset.parquet` (2,765 rows, the canonical current dataset), produced by `notebooks/03_event_detection.ipynb` §5 (renaming/deriving `gdelt_risk_score = gdelt_mean_goldstein / 10` and `gdelt_n_events = gdelt_event_count` from the Notebook 01 daily summary) and merged in by `scripts/build_master_dataset.py`.
3. **The substantive claim is false for the current data, not merely uncited.** Directly inspecting `data/processed/master_dataset.parquet` today: `gdelt_risk_score` mean=0.0593, std=0.0147; `gdelt_mean_tone` mean=−1.945, std=0.506; `gdelt_n_events` mean=163,267, std=44,672. None are zero-variance. This is expected — Notebook 01 now performs a genuine 2015–2025 GDELT backfill (4,018 daily rows, verified in `data/raw/gdelt_daily_summary.parquet`), not the earlier 5-day sample the 2026-07-05 entry was consistent with at the time it was written.

**Interpretation:** The withdrawn statement was accurate for the pipeline state that existed on 2026-07-05 (GDELT limited to a 5-day out-of-window sample, per the same day's decision-log entry), but was never corrected or re-cited after the 2026-07-13 GDELT full-history backfill superseded it, and its source citation was imprecise even at the time (Notebook 02 never performed this computation). No RQ1, RQ2, or RQ3 conclusion in this log depends on the withdrawn statement — GDELT remains, by deliberate design, outside the frozen causal DAG and FES v1.1 feature matrix (`10_decision_log.md`, 2026-07-13), so its variance (zero or otherwise) does not affect any reported RQ result.

**Also corrected in this pass:** `notebooks/01_data_collection.ipynb`'s GDELT section now explicitly separates (A) loading the frozen `gdelt_daily_summary.parquet`, (B) rebuilding it from a raw file, and (C) refreshing raw data from GDELT, defaulting to (A) only; a branch-condition defect that tested for one file but read another was fixed; and the previous silent all-zero GDELT fallback was replaced with a clear error. See `10_decision_log.md` for the full change record.

**Reversible:** Yes. This is a documentation correction; no dataset, model, or reported statistic changed.
