...
**Causal DAG** (see `docs/architecture/causal_dag_dowhy_professional_clean.svg`): Treatment = event sentiment (lexicon score, primary; FinBERT score, robustness check) → Outcome = next-day SPY return. Confounders: VIX regime, prior-day market return.
...
**Sentiment scoring — two methods evaluated in parallel:**
1. **FinBERT** (`ProsusAI/finbert`, via `src/event_detector.py::EventDetector`) — a transformer pretrained on financial-news sentiment, scored on document *titles* (not full text — see `11_limitations.md`). Parameters from `config.yaml: nlp` (batch_size 32, max_length 512, confidence threshold 0.7).
2. **Lexicon scorer** — a curated financial/political keyword lexicon, tuned for policy-register language rather than financial-news register.

**Method selection:** The lexicon scorer is used as the **primary** sentiment signal in the causal model and feature set; FinBERT is retained as a **robustness check** only. Reason: FinBERT's title-level output on formal presidential language is 95.3% neutral (vs. 73.1% neutral for the lexicon method) — a domain-mismatch effect (FinBERT was trained on financial-news headlines, not policy speech), which leaves too little discriminative signal for the lexicon method's role in this pipeline. This is a logged decision — see `10_decision_log.md`.
