...
6. **`QQQ`/`GLD`/`TLT` and `gdelt_*` columns carry their limitations forward.** Any notebook consuming these columns must repeat the caveat already stated in `dataset_version.md` (cross-asset columns unused pending a scoped decision; GDELT columns are a 5-day sample) — do not treat their mere presence in the schema as license to use them in a "final" RQ1–RQ3 result without first checking `10_decision_log.md`/`11_limitations.md` L7/L8 have been closed.
...
10. **Version pinning.** Any notebook or script reading this file should state which version it was validated against (currently v1.0) in its header/purpose cell — if `dataset_version.md` shows a newer version exists, re-validate the notebook's outputs before trusting them, rather than assuming forward compatibility.

---

## Consumers of this contract (as of 2026-07-04)

| Consumer | Relationship |
|---|---|
| `05_feature_engineering.ipynb` | Primary consumer — builds `model_features.parquet` from this file (Mission 05, not yet rebuilt under this contract) |
...
- [x] Consumption caveats for QQQ/GLD/TLT and GDELT carried forward rather than re-litigated
