# Baseline Model Specification — `Baseline_LASSO` v1.1
...
`feature_matrix.parquet` FES v1.1, rows where `split == "train"` (1,727 rows, 2016-02-24 → 2022-12-30), restricted to the 27 **Market**-category columns (`feature_contract.md` Baseline Eligibility table). No Macro, Sentiment, Event, Temporal, or Interaction column is read. Notebook 06 verifies the input's validation status, version, row count, feature count, and SHA-256 before fitting.
...
|---|---|---|
| `alpha` (L1 regularisation strength) | **0.0018533887501907256** | `LassoCV`'s automatic coordinate-descent path search, minimising mean cross-validated squared error across a ~100-value alpha grid, evaluated via the `TimeSeriesSplit(5)` folds above |
...
At the CV-selected `alpha = 0.0018533887501907256`, **all 27 Market-feature coefficients shrink to exactly zero**; the fitted model reduces to its intercept alone (`0.00046584536225195564`, the training-target mean). This is not a bug — verified directly against the fitted `coef_` array (27/27 exactly `0.0`) — and is not adjusted, cherry-picked, or re-tuned to avoid it. The dated FES v1.0 archive produces the same alpha, intercept, coefficient state, and core metrics within the frozen tolerance because FES v1.1 changed no Market value, target, date, or split. See `baseline_evaluation.md` for the RQ3 interpretation.
