...
| `causal_engine.py` | Event study (abnormal return / CAR) and DoWhy causal-graph estimation helpers | **Corrected 2026-07-13** — not actually imported by any current notebook. `04_causal_analysis.ipynb` implements its own inline `EventStudy` class rather than importing this module (confirmed via grep — no `from src.causal_engine import` anywhere in that notebook); the archived pre-rebuild Notebook 3 only mentioned the class in prose. Same "historically used, now inline in-notebook" pattern as `features.py`/`models.py`/`evaluation.py` below — this was previously mis-stated as an active dependency. Its `EventStudy` class (an OLS market-model, not Notebook 04's actual constant-mean-return method) is tested by `tests/test_legacy_causal_engine.py`, clearly labelled legacy. |
| `event_study_reference.py` | **Added 2026-07-15.** Dependency-free reference implementation of Notebook 04's *actual* constant-mean-return event-study calculation (same estimation/event windows, same 60-observation minimum) — not a rewrite, a parallel implementation for test coverage. Cross-validated to <1e-6 against all 264 rows of the frozen `car_results.parquet`. | Test-only: `tests/test_event_study.py`. **Not imported by `04_causal_analysis.ipynb`** — the notebook's inline `compute_car` remains authoritative and unchanged; see `docs/research_bible/10_decision_log.md`, 2026-07-15 entry for why no refactor was made despite exact numerical equivalence. |
