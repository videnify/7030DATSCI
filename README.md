...
- Trained models (`models/`) are binaries and are also excluded from version control; they are regenerated by `06_model_training.ipynb` (or `07_machine_learning.ipynb` per the 10-phase spec).
...
> **Note:** FinBERT was trained on financial news headlines; the high neutral rate (95.3%) reflects domain mismatch with formal presidential language. The curated lexicon scorer — tuned with financial and political keywords — produces a more discriminative distribution and is preferred for policy text analysis.
...
### Method
Event study (AR/CAR, window −5 to +10 trading days) + DoWhy backdoor linear regression with confounders: VIX regime, prior-day return. Primary treatment: lexicon sentiment. Robustness check: FinBERT sentiment.
...
- **DoWhy causal effect (lexicon):** +0.005046 per unit sentiment, 95% CI [+0.0014, +0.0087]
