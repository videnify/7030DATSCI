"""
build_master_dataset.py — Dedicated freeze step for data/processed/master_dataset.parquet
============================================================================================
master_dataset.parquet (v1.0, frozen 2026-07-04) was built once, outside any notebook
currently in this repository, and is no longer present on disk. Per dataset_contract.md
term 1 ("read-only... a schema or content change requires a new version built by a
dedicated freeze step") and term 2 ("05_feature_engineering.ipynb must not independently
re-derive the raw price/VIX/macro/sentiment merge"), the fix is to rebuild this file here,
as its own standalone step, matching dataset_version.md's documented v1.0 schema exactly
-- not to have 05_feature_engineering.ipynb quietly re-derive the merge itself.

This is a v1.1 rebuild: the schema and construction rules are unchanged from v1.0, but the
underlying daily_sentiment.parquet (Notebook 03 output) has genuinely changed composition
since v1.0 was frozen (916-document economic pre-filter, FinBERT-only, no lexicon; GDELT
is now a full 4,018-day daily series instead of a 5-day sample). See 10_decision_log.md
(2026-07-13 entry) for the full reasoning.
...
print("\nValidation:")
n_dupe_dates = master["date"].duplicated().sum()
is_monotonic = master["date"].is_monotonic_increasing
